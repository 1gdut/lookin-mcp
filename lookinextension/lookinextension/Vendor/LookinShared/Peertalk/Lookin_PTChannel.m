...
- (void)connectToPort:(in_port_t)port host:(NSString *)host callback:(void(^)(NSError *error, Lookin_PTAddress *address))callback {
  assert(protocol_ != NULL);
  if (connState_ != kConnStateNone) {
    if (callback) callback([NSError errorWithDomain:NSPOSIXErrorDomain code:EPERM userInfo:nil], nil);
    return;
  ...
  if (host.length == 0) {
    if (callback) callback([NSError errorWithDomain:@"PTError" code:0 userInfo:@{NSLocalizedDescriptionKey: @"Missing host"}], nil);
    return;
  }
  connState_ = kConnStateConnecting;
  ...
  int gaiResult = getaddrinfo(host.UTF8String, portText, &hints, &addresses);
  if (gaiResult != 0 || !addresses) {
    connState_ = kConnStateNone;
    NSString *message = [NSString stringWithFormat:@"Failed to resolve host %@: %s", host, gai_strerror(gaiResult)];
    if (callback) callback([NSError errorWithDomain:@"PTError" code:gaiResult userInfo:@{NSLocalizedDescriptionKey: message}], nil);
    return;
  }

  dispatch_fd_t connectedFD = -1;
  struct sockaddr_storage connectedAddr;
  bzero(&connectedAddr, sizeof(connectedAddr));
  NSError *connectError = nil;

  for (struct addrinfo *info = addresses; info != NULL; info = info->ai_next) {
    dispatch_fd_t fd = socket(info->ai_family, info->ai_socktype, info->ai_protocol);
    if (fd == -1) {
      connectError = [[NSError alloc] initWithDomain:NSPOSIXErrorDomain code:errno userInfo:nil];
      continue;
    }
    ...
    if (connect(fd, info->ai_addr, info->ai_addrlen) == 0) {
      connectedFD = fd;
      memcpy(&connectedAddr, info->ai_addr, MIN(sizeof(connectedAddr), (size_t)info->ai_addrlen));
      break;
    }
  ...
  if (connectedFD == -1) {
    connState_ = kConnStateNone;
    if (callback) callback(connectError ?: [NSError errorWithDomain:@"PTError" code:0 userInfo:nil], nil);
    return;
  }

  dispatch_io_t dispatchChannel = dispatch_io_create(DISPATCH_IO_STREAM, connectedFD, protocol_.queue, ^(int error) {
    close(connectedFD);
    if (self->delegateFlags_ & kDelegateFlagImplements_ioFrameChannel_didEndWithError) {
      NSError *err = error == 0 ? self->endError_ : [[NSError alloc] initWithDomain:NSPOSIXErrorDomain code:error userInfo:nil];
      [self->delegate_ ioFrameChannel:self didEndWithError:err];
  ...
  NSError *err = nil;
  Lookin_PTAddress *ptAddr = [[Lookin_PTAddress alloc] initWithSockaddr:&connectedAddr];
  [self startReadingFromConnectedChannel:dispatchChannel error:&err];
  if (callback) callback(err, ptAddr);
}
